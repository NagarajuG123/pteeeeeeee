...
export class LatestStoryComponent implements OnInit {
  ...
  getMoreItem() {
    this.apiService
      .getAPI(
        `${this.slug}/brand-latest-stories?limit=8&offset=${this.items.length}`
      )
      .subscribe((result) => {
  ...
  readMore(item: any) {
    return this.commonService.readMore1(item, 'latest-story');
  }
  getLatestStory() {
    this.apiService
      .getAPI(
        `${this.slug}/brand-latest-stories?limit=8&offset=${this.scrollOffset}`
      )
      .subscribe((response) => {
