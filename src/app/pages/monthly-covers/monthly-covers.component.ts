...
export class MonthlyCoversComponent implements OnInit {
  ...
  ngOnInit(): void {
    if(this.tstate.hasKey(RESULT_KEY)){
      ...
      const coverData = {};
      const coverApi = this.apiService.getAPI(`1851/journal/monthly-covers?limit=14&offset=0`);
      const metaApi = this.apiService.getAPI(`1851/meta`);
      const publicationApi = this.apiService.getAPI(`1851/publication-instance`);

     forkJoin([coverApi,metaApi,publicationApi]).pipe(takeUntil(this.onDestroy$)).subscribe((response ) =>{
        ...
        this.metaService.setTitle(`Monthly Issues | ${coverData['publicationTitle']}`);
      });
